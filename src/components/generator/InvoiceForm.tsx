...
export default function InvoiceForm({ signedIn }: InvoiceFormProps) {
  ...
  // Handle URL parameters from quick start form
  useEffect(() => {
    const fromParam = searchParams.get('from');
    const emailParam = searchParams.get('email');
    const vatParam = searchParams.get('vat');
    const vatModeParam = searchParams.get('vatMode');

    if (fromParam) {
      setSender(prev => ({ ...prev, company: fromParam }));
    }

    if (vatParam) {
      setSender(prev => ({ ...prev, vat: vatParam }));
    }

    if (emailParam) {
      setClient(prev => ({ ...prev, email: emailParam }));
    }

    if (vatModeParam) {
      // Convert vatMode parameter back to TaxMode
      const modeMap: Record<string, TaxMode> = {
        'domestic': 'domestic',
        'intra-eu': 'intraEU_rc',
        'export': 'export'
      };
      const taxMode = modeMap[vatModeParam] || 'domestic';
      setTaxMode(taxMode);
    }
  }, [searchParams]);
